...
async def get_root(request: Request):
    ...
    if usuario.perfil == 2:
        return RedirectResponse("/artesaos", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/post_entrar_cliente")
...
@router.get("/entrar_artesaos")
async def get_root(request: Request):
    usuario = request.state.usuario if hasattr(request.state, "usuario") else None
    if not usuario:
       return templates.TemplateResponse("/entrar", {"request": request})
    if usuario.perfil == 1:
        return RedirectResponse("/cliente", status_code=status.HTTP_303_SEE_OTHER)
    if usuario.perfil == 2:
        return RedirectResponse("/artesaos", status_code=status.HTTP_303_SEE_OTHER)

@router.post("/post_entrar_artesaos")
async def post_entrar(
    email: str = Form(...), 
    senha: str = Form(...)):
    usuario = UsuarioRepo.checar_credenciais(email, senha)
